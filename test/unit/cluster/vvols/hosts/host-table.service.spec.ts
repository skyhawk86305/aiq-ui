...
describe('HostTableService', function () {
  ...
  beforeEach(inject(function ($q, $rootScope, HostTableService, DataService, SFTableService) {
    rootScope = $rootScope;
    deferred = $q.defer();
  ...
  describe('.getData (inherited from SFTableService)', function() {
    it('should call the appropriate API method with the selectedClusterID', function() {
      ...
      expect(dataService.callGuzzleAPI).toHaveBeenCalledWith('ListVirtualVolumeHosts', {clusterID: 'foobar'});
    });
