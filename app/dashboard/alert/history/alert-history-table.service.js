(function () {
  ...
  function AlertHistoryTableService(SFTableService, SFFilterComparators, DataService, $filter) {
    ...
    var columns = [
      ...
      {key: 'created', label: 'Alert Triggered', format: {filter: 'date', args:['yyyy-MM-dd HH:mm:ss']}},
      {key: 'lastNotified', label: 'Last Notification', format: {filter: 'date', args:['yyyy-MM-dd HH:mm:ss']}},
      {key: 'isResolved', label: 'Resolved', format: {filter: 'boolean', args:['Yes', 'No']}},
      {key: 'resolved', label: 'Resolution Time', format: {filter: 'date', args:['yyyy-MM-dd HH:mm:ss']}},
